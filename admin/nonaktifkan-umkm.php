...
if (isset($_GET['toggle_status'])) {
    $id_toggle = $_GET['toggle_status'];
    $status_baru = mysqli_real_escape_string($koneksi, $_GET['status'] ?? '');

    if (in_array($status_baru, ['aktif', 'nonaktif'])) {
        mysqli_query($koneksi, "UPDATE penjual SET status='$status_baru' WHERE id_penjual='$id_toggle'");
    }
    header("Location: index.php?page=admin/nonaktifkan-umkm");
    exit;
}
